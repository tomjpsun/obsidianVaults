...
//#if DM9051_1588
	...
	u8  ptp_tx_flags, ptp_msgtype; //Stone add for DM9051A 1588 one-step sync! 2024-08-16
...
#endif
* */
    //Stone add for DM9051A 1588 one-step sync packet! 2024-08-16
    if (db->ptp_tx_flags == 1)
	{
	...
	while (!skb_queue_empty(&db->txq)) {
		...
		if (skb) {
			...
			if (skb) { //identical to.
				...
				//printk("### skb_shinfo(skb)->tx_flags = %x ###\r\n", db->ptp_tx_flags);
